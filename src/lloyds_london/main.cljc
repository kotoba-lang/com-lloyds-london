(ns lloyds_london.main
  "Kotodama WASM entrypoint for the Lloyds_london clean-room actor (L4) — Clojure port.
  ...
  facts (`lloyds_london.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "lloyds_london")
(def tier "L4")
...
(def entity-specs
  [{:entity "Account"   :plural "accounts"   :id-prefix "lloydslo_acc"
    :fields [:holderId :type :balance :currency :status] :required [:type :balance]
    :coerce {:balance :float}                :refs {}}
   {:entity "Loan"      :plural "loans"      :id-prefix "lloydslo_loa"
...
(defn emit-facts
  "EAVT facts for one record: {\"lloyds_london.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "lloyds_london-compat" :tier tier :entities entities} 200])
...
(defn start! [] :lloyds_london-compat/ready)
